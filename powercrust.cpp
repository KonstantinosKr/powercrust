...
class powercrustException : public std::exception
{

public:
  powercrustException ( const char* m = "powercrustException!" ) : msg ( m ) { }
  ~powercrustException() throw() {}
...
class powercrustImpl
{

private:

  typedef void* ( powercrustImpl::*visit_func ) ( simplex *, void * );
  typedef int ( powercrustImpl::*test_func ) ( simplex *, int, void * );
...
#define  RAND48_ADD  (0x000b)
  ...
  long ( powercrustImpl::*shuf ) ( long );
  long ( powercrustImpl::*site_num ) ( site );
  site ( powercrustImpl::*get_site ) ( );
  site ( powercrustImpl::*get_site_n ) ( long );

  site get_next_site ( void );

  void make_output ( simplex *root, void * ( powercrustImpl::*visit_gen ) ( simplex*, visit_func ), visit_func visit );
...
public:
  ...
  powercrust *our_filter;

};

//=====================================================================

powercrustImpl::powercrustImpl ( void )
{
}

long powercrustImpl::site_numm ( site p )
{
...
site powercrustImpl::new_site ( site p, long j )
{
...
void powercrustImpl::read_bounding_box ( long j )
{
  int i,k;
...
site powercrustImpl::vtk_read_next_site ( long j )
{
...
site powercrustImpl::vtk_pole_read_next_site ( long j )
{
...
site powercrustImpl::get_site_offline ( long i )
{
...
void powercrustImpl::make_shuffle ( void )
{
...
long powercrustImpl::shufflef ( long i )
{
  return shufmat[i];
}

site powercrustImpl::get_next_site ( void )
{
  return ( this->*get_site_n ) ( ( this->*shuf ) ( s_num++ ) );
}

void powercrustImpl::make_output ( simplex *root, void * ( powercrustImpl::*visit_gen ) ( simplex*, visit_func ), visit_func visit )
{
//   ( this->*visit ) ( 0, ( void ( * ) ) out_funcp );
  ( this->*visit_gen ) ( root, visit );
}

void powercrustImpl::ASSERT ( int b, const char* message )
{
  if ( !b )
    {
...
void powercrustImpl::freeAll ( void )
{
...
void powercrustImpl::adapted_main ( double m_mult_up )
{
  int num_poles = 0;
  ...
  init_rand();
  make_shuffle();
  shuf = &powercrustImpl::shufflef;
  get_site_n = &powercrustImpl::get_site_offline;
  ...
  pr = &powercrustImpl::compute_vv;
  make_output ( root, &powercrustImpl::visit_hull, pr );

  pr = &powercrustImpl::compute_pole2;
  make_output ( root, &powercrustImpl::visit_hull, pr );
  ...
  make_shuffle();
  shuf = &powercrustImpl::shufflef;
  get_site_n = &powercrustImpl::get_site_offline;  /* returns stored points, unshuffled */

  /* Compute weighted DT  */
  root = build_convex_hull ( dim, vd_new );

  /* compute adjacencies and find angles of ball intersections */
  pr = &powercrustImpl::compute_3d_power_vv;
  make_output ( root, &powercrustImpl::visit_hull, pr );
  ...
  pr = &powercrustImpl::compute_axis;
  make_output ( root, &powercrustImpl::visit_hull, pr );
}

void powercrustImpl::compute_distance ( simplex** poles, int size, double* distance )
{
...
void powercrustImpl::_dorand48 ( unsigned short xseed[3] )
{
  unsigned long accu;
...
double powercrustImpl::local_erand48 ( unsigned short xseed[3] ) throw()
{
  _dorand48 ( xseed );
  return ldexp ( ( double ) xseed[0], -48 ) + ldexp ( ( double ) xseed[1], -32 ) + ldexp ( ( double ) xseed[2], -16 );
}

double powercrustImpl::double_rand ( void )
{
  return local_erand48 ( X );
}

void powercrustImpl::init_rand ( void )
{
  X[1] = 10000;
}

double powercrustImpl::logb ( double x )
{
  return log ( x ) /log ( 2.0 ); // EPRO added
}

REAL powercrustImpl::orient3dadapt ( REAL *pa,REAL *pb,REAL *pc,REAL *pd,REAL permanent )
{
...
REAL powercrustImpl::orient3d ( REAL *pa,REAL *pb,REAL *pc,REAL *pd )
{
...
REAL powercrustImpl::estimate ( int elen,REAL *e )
{
  REAL Q;
  int eindex;
...
REAL powercrustImpl::orient2dadapt ( REAL *pa,REAL *pb,REAL *pc,REAL detsum )
{
...
int powercrustImpl::scale_expansion_zeroelim ( int elen, REAL *e, REAL b, REAL *h )
{
  INEXACT REAL Q, sum;
  REAL hh;
...
int powercrustImpl::fast_expansion_sum_zeroelim ( int elen,REAL *e,int flen,REAL *f,REAL *h )
{
  REAL Q;
  INEXACT REAL Qnew;
  INEXACT REAL hh;
...
void powercrustImpl::tetorthocenter ( double a[4],double b[4],double c[4],double d[4],double orthocenter[3], double *cnum )
{
...
void powercrustImpl::tetcircumcenter ( double a[3],double b[3],double c[3],double d[3], double circumcenter[3],double *cond )
{
...
double powercrustImpl::sqdist ( double a[3], double b[3] )
{
  /* returns the squared distance between a and b */
  return SQ ( a[0]-b[0] ) +SQ ( a[1]-b[1] ) +SQ ( a[2]-b[2] );
}

void powercrustImpl::dir_and_dist ( double a[3], double b[3], double dir[3], double* dist )
{
  int k;
...
void powercrustImpl::opp_update ( int pi )
{
  plist *pindex;
...
void powercrustImpl::sym_update ( int pi )
{
  edgesimp *eindex;
...
void powercrustImpl::update_pri ( int hi, int pi )
{
  double pr;
...
void powercrustImpl::label_unlabeled ( int num )
{
  plist *pindex;
  edgesimp *eindex;
  int opplabel;
...
neighbor * powercrustImpl::op_simp ( simplex *a, simplex *b )
{
  lookup ( a, b, simp )
}

neighbor * powercrustImpl::op_vert ( simplex *a, site b )
{
  lookup ( a, b, vert )
}

void powercrustImpl::connect ( simplex *s )
{
...
simplex * powercrustImpl::make_facets ( simplex *seen )
{
...
simplex * powercrustImpl::extend_simplices ( simplex *s )
{
...
simplex * powercrustImpl::search ( simplex *root )
{
...
point powercrustImpl::get_another_site ( void )
{
  point pnext;
...
void powercrustImpl::buildhull ( simplex *root )
{
  while ( cdim < rdim )
    {
      p = get_another_site();
...
int powercrustImpl::reduce ( basis_s **v, point p, simplex *s, int k )
{
  point   z;
...
void powercrustImpl::get_basis_sede ( simplex *s )
{
  int k=1;
...
int powercrustImpl::out_of_flat ( simplex *root, point p )
{
  if ( !p_neigh.basis )
...
double powercrustImpl::cosangle_sq ( basis_s* v, basis_s* w )
{
  double dd;
...
int powercrustImpl::check_perps ( simplex *s )
{
  point z, y;
  point tt;
...
void powercrustImpl::get_normal_sede ( simplex *s )
{
  neighbor *rn;
  int i, j;
...
void powercrustImpl::get_normal ( simplex *s )
{
  get_normal_sede ( s );
  return;
}

int powercrustImpl::sees ( site p, simplex *s )
{
  point tt, zz;
  double dd, dds;
  int i;
...
double powercrustImpl::radsq ( simplex *s )
{
  point n;
  neighbor *sn;
  int i;
...
void * powercrustImpl::zero_marks ( simplex * s, void *dum )
{
  s->mark = 0;
  return NULL;
}

void * powercrustImpl::one_marks ( simplex * s, void *dum )
{
  s->mark = 1;
  return NULL;
}

int powercrustImpl::alph_test ( simplex *s, int i, void *alphap )
{
...
void * powercrustImpl::conv_facetv ( simplex *s, void *dum )
{
...
void * powercrustImpl::mark_points ( simplex *s, void *dum )
{
  int i, snum;
  neighbor *sn;
...
void * powercrustImpl::visit_outside_ashape ( simplex *root, visit_func visit )
{
  return visit_triang_gen ( ( simplex* ) visit_hull ( root, &powercrustImpl::conv_facetv ), visit, &powercrustImpl::alph_test );
}

int powercrustImpl::check_ashape ( simplex *root, double alpha )
{
  for ( int i = 0; i < MAXPOINTS; i++ )
    {
      mi[i] = mo[i] = 0;
  ...
  visit_hull ( root, &powercrustImpl::zero_marks );

  alph_test ( 0, 0, &alpha );
  visit_outside_ashape ( root, &powercrustImpl::one_marks );

  visit_hull ( root, &powercrustImpl::mark_points );
...
simplex * powercrustImpl::build_convex_hull ( short dim, short vdd )
{
  /*
  ...
  */
  ...
  cdim = 0;
  get_site = &powercrustImpl::get_next_site;
  site_num = &powercrustImpl::site_numm;
  pdim = dim;
  vd = vdd;
...
void powercrustImpl::free_hull_storage ( void )
{
...
void * powercrustImpl::compute_vv ( simplex *s, void *p )
{
...
void * powercrustImpl::compute_pole2 ( simplex *s, void *p )
{
  point v[MAXDIM];
  int inf = 0;
  double a[3] = {0, 0, 0};
...
int powercrustImpl::close_pole ( double* v, double* p, double lfs_lb )
{
  return ( sqdist ( v, p ) < lfs_lb * lfs_lb );
}

int powercrustImpl::antiLabel ( int label )
{
...
double powercrustImpl::computePoleAngle ( simplex* pole1, simplex* pole2, double* samp )
{
...
void powercrustImpl::newOpposite ( int p1index, int p2index, double pole_angle )
{
  plist* newplist;
...
void powercrustImpl::outputPole ( simplex* pole, int poleid, double* samp, int* num_poles,double distance )
{
...
Tree * powercrustImpl::splay ( site i, Tree *t )
{
  Tree N, *l, *r, *y;
...
Tree * powercrustImpl::insert ( site i, Tree * t )
{
...
void powercrustImpl::free_heap ()
{
  free ( heap_A );
}

void powercrustImpl::init_heap ( int num )
{
...
void powercrustImpl::heapify ( int hi )
{
  int largest;
...
int powercrustImpl::extract_max()
{
...
int powercrustImpl::insert_heap ( int pi, double pr )
{
  heap_size++;
  int i = heap_size;
...
void powercrustImpl::update ( int hi, double pr )
{
  heap_A[hi].pri = pr;
...
void * powercrustImpl::visit_triang_gen ( simplex *s, visit_func visit, test_func test )
{
...
int powercrustImpl::truet ( simplex *s, int i, void *dum )
{
  return 1;
}

void * powercrustImpl::visit_triang ( simplex *root, visit_func visit )
{
  return visit_triang_gen ( root, visit, &powercrustImpl::truet );
}

int powercrustImpl::hullt ( simplex *s, int i, void *dummy )
{
  return i > -1;
}

void * powercrustImpl::facet_test ( simplex *s, void *dummy )
{
  return ( !s->peak.vert ) ? s : NULL;
}

void * powercrustImpl::visit_hull ( simplex *root, visit_func visit )
{
  return visit_triang_gen ( ( simplex* ) visit_triang ( root, &powercrustImpl::facet_test ), visit, &powercrustImpl::hullt );
}

Coord powercrustImpl::Vec_dot ( point x, point y )
{
  Coord sum = 0;
...
Coord powercrustImpl::Vec_dot_pdim ( point x, point y )
{
  Coord sum = 0;
...
Coord powercrustImpl::Norm2 ( point x )
{
  Coord sum = 0;
...
void powercrustImpl::Ax_plus_y ( Coord a, point x, point y )
{
...
void powercrustImpl::Ax_plus_y_test ( Coord a, point x, point y )
{
  for ( int i = 0; i < rdim; i++ )
    {
      *y++ += a * *x++;
...
void powercrustImpl::Vec_scale_test ( int n, Coord a, Coord *x )
{
...
double powercrustImpl::sc ( basis_s *v,simplex *s, int k, int j )
{
...
int powercrustImpl::reduce_inner ( basis_s *v, simplex *s, int k )
{
...
void powercrustImpl::trans ( point z, point p, point q )
{
  for ( int i = 0; i < pdim; i++ )
    z[i + rdim] = z[i] = p[i] - q[i];
}

void * powercrustImpl::compute_3d_power_vv ( simplex *s, void *p )
{
  point v[MAXDIM];
  int inf = 0;
  int index = 0;
...
void * powercrustImpl::compute_axis ( simplex *s, void *p )
{
  point v[MAXDIM];
  point  point1, point2;
  int edgedata[6];
...
void powercrustImpl::construct_face ( simplex *s, short k )
{
...
int powercrustImpl::correct_orientation ( double *p1,double *p2,double *p3,double *inp,double *outp )
{
  double normal[3];
  double v1[3],v2[3];
...
void powercrust::Error ( const char *message )
{
  throw powercrustException ( message );
}

int powercrust::FillInputPortInformation ( int, vtkInformation *info )
{
  info->Set ( vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkPolyData" );
  return 1;
}

int powercrust::RequestData ( vtkInformation* vtkNotUsed ( request ), vtkInformationVector** InputVector, vtkInformationVector* OutputVector )
{
  powercrustImpl PowerCrustImpl;
...
void powercrust::ExecuteInformation()
{
